...
public class AsyncXmlLoader extends AsyncTask<Void, String, Void> {
	...
	protected void onPostExecute(final Void result) {
		...
		if (isCancelled()) {
			mListener
					.onXmlDocumentLoadError(uri, new CancellationException(), null);
		}

		if (mThrowable == null) {
			mListener.onXmlDocumentLoaded(mRoot, uri, mHash, mEncoding,
					mForceReadOnly);
		} else {
			mListener.onXmlDocumentLoadError(uri, mThrowable, null);
		}
	 ...
	 */
	protected void doParseDocument() throws StringIndexOutOfBoundsException,
	 ...
	 * @throws StringIndexOutOfBoundsException
	 */
	protected void doParseXmlDocument() throws StringIndexOutOfBoundsException,
			XmlPullParserUnavailableFeatureException,
			XmlPullParserInstantiationException, XmlPullParserException,
			IOException {
	 ...
	 * @throws IOException
	 */
	protected void doParseCompressedDocument() throws IOException {
		InputStream input = getInputStream();
