...
import fr.xgouchet.xmleditor.ui.widget.UndoBarController;
...
public class AxelActivity extends Activity implements XmlEditorListener {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        getFragmentManager().beginTransaction().add(R.id.content, mEditorFragment).commit();
        ...
        // Setup undo bar controller
        mUndoBarController = new UndoBarController(findViewById(R.id.frame_notification));
    }
    ...
    protected void onStart() {
        ...
        if (changeLog.displayChangeLog(this)) {
            // copy templates from the assets
            //TODO  TemplateFiles.copyTemplatesFromAssets(this);
        }
    ...
    public boolean onPrepareOptionsMenu(final Menu menu) {
        ...
        for (File template : templateFiles) {
            templates.add(R.id.action_group_template, 0, Menu.NONE, template.getName());
        }
        ...
        for (RecentEntry entry : recentEntries) {
            MenuItem item = recents.add(R.id.action_group_recents,
                    (int) (entry.getTimestamp() / 1000L), Menu.NONE, entry.getName());
    ...
    private void saveDocumentAs() {
        ...
        if (!TextUtils.isEmpty(mXmlEditor.getCurrentDocumentName())) {
            save.putExtra(Intent.EXTRA_TITLE, mXmlEditor.getCurrentDocumentName());
        }
    ...
    private void promptSaveIfDirty(final boolean allowNoSave) {
        ...
        PromptDialogHelper.promptDirtyDocumentAction(this, listener, allowNoSave);
    }
    ...
    private void updateTitle() {
        ...
        int titleId = mXmlEditor.isReadOnly() ? R.string.title_editor_readonly : (mXmlEditor
                .isDirty() ? R.string.title_editor_dirty : R.string.title_editor);
    ...
    @Override
    public void onConfirmNotification(final String message, final Runnable undoAction) {
        if (undoAction == null) {
            Crouton.makeText(this, message, Style.CONFIRM).show();
        } else {
            mUndoBarController.showUndoBar(false, message, undoAction);
        }
    }
